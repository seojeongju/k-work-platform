-- 테스트 계정 생성 SQL (idempotent, FK-safe)

-- 1) 관리자 계정 (중복 시 무시)
INSERT OR IGNORE INTO admins (email, password, name, role, status, created_at)
VALUES ('admin@wowcampus.com', 'admin123', 'WOW-CAMPUS 관리자', 'super_admin', 'active', CURRENT_TIMESTAMP);

-- 2) 에이전트 계정들 (중복 시 무시)
INSERT OR IGNORE INTO agents (email, password, company_name, contact_person, phone, country, status, created_at) VALUES 
...
-- 3) 구인기업 계정들 (고유 business_number 사용, 중복 시 무시)
INSERT OR IGNORE INTO employers (email, password, company_name, business_number, industry, contact_person, phone, address, region, status, created_at) VALUES 
('hr@techcorp.com', 'company123', '테크코퍼레이션', '999-99-00001', 'IT/소프트웨어', '김인사', '02-1111-2222', '서울시 강남구 테헤란로 123', '서울', 'approved', CURRENT_TIMESTAMP),
('jobs@manufacturing.com', 'company123', '한국제조', '999-99-00002', '제조업', '박채용', '031-3333-4444', '경기도 안산시 공단로 456', '경기', 'approved', CURRENT_TIMESTAMP),
('recruit@globalservice.com', 'company123', '글로벌서비스', '999-99-00003', '서비스업', '최담당', '02-5555-6666', '서울시 마포구 월드컵북로 789', '서울', 'approved', CURRENT_TIMESTAMP);

-- 4) 구직자 계정들 (agent_id는 이메일을 기준으로 조회해 FK 안전)
...
(
    ...
    '학사', '베트남에서 웹개발 2년 경험', (SELECT id FROM agents WHERE email='agent1@wowcampus.com'), 'active', CURRENT_TIMESTAMP
),
(
    'maria@example.com', 'jobseeker123', '마리아 산토스', '1992-07-20', 'female', '필리핀', 
    '010-2222-2222', 'H-2', 'F-4', 'advanced', '인천시 남동구', 
    '전문학사', '호텔 서비스업 3년 경험', (SELECT id FROM agents WHERE email='agent2@wowcampus.com'), 'active', CURRENT_TIMESTAMP
),
(
    'somchai@example.com', 'jobseeker123', '솜차이', '1988-12-10', 'male', '태국', 
    '010-3333-3333', 'E-9', 'H-2', 'beginner', '경기도 화성시', 
    '고등학교', '태국에서 자동차 부품 제조 5년', (SELECT id FROM agents WHERE email='agent3@wowcampus.com'), 'active', CURRENT_TIMESTAMP
),
(
    'ahmed@example.com', 'jobseeker123', '아흐메드', '1990-05-25', 'male', '방글라데시', 
    '010-4444-4444', 'E-9', 'H-2', 'intermediate', '경기도 안산시', 
    '학사', '건설 현장 관리 4년 경험', (SELECT id FROM agents WHERE email='agent1@wowcampus.com'), 'active', CURRENT_TIMESTAMP
);

-- 5) 구인공고들 (employer_id는 이메일 기준 조회로 FK 안전)
